...
static int fread_bycol(FILE *const f, const size_t m, const size_t n, double *const A, const size_t ldA) throw()
{
  if (!f)
    return -1;
  if (!m)
...
static int fwrite_bycol(FILE *const f, const size_t m, const size_t n, const double *const A, const size_t ldA) throw()
{
  if (!f)
    return -1;
  if (!m)
